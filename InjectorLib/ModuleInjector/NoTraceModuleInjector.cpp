...
BOOL libTools::CNoTraceModuleInjector::Hook()
{
	return CInlineHook::Hook(_NtOpenFilePtr, reinterpret_cast<LPVOID>(_NtOpenFile), reinterpret_cast<LPVOID*>(&_NtOpenFilePtr_Old)) &&
		...
		CInlineHook::Hook(_NtQueryAttributesFilePtr, reinterpret_cast<LPVOID>(_NtOpenFile), reinterpret_cast<LPVOID*>(&_NtQuerySectionPtr_Old)) &&
		CInlineHook::Hook(_NtMapViewOfSectionPtr, reinterpret_cast<LPVOID>(_NtOpenFile), reinterpret_cast<LPVOID*>(&_NtMapViewOfSectionPtr_Old)) &&
		CInlineHook::Hook(_NtClosePtr, reinterpret_cast<LPVOID>(_NtOpenFile), reinterpret_cast<LPVOID*>(&_NtClosePtr_Old)) &&
		CInlineHook::Hook(_NtQueryAttributesFilePtr, reinterpret_cast<LPVOID>(_NtOpenFile), reinterpret_cast<LPVOID*>(&_NtQueryAttributesFilePtr_Old));
}
...
NTSTATUS WINAPI libTools::CNoTraceModuleInjector::_NtOpenFile(OUT PHANDLE FileHandle, IN ACCESS_MASK DesiredAccess, IN POBJECT_ATTRIBUTES ObjectAttributes, OUT PIO_STATUS_BLOCK IoStatusBlock, IN ULONG ShareAccess, IN ULONG OpenOptions)
{
	...
	if (!_RtlCompareUnicodeStringPtr(ObjectAttributes->ObjectName, &pMemoryModule->MemDllFullPath, TRUE))
	{
		return _NtOpenFilePtr_Old(FileHandle, DesiredAccess, ObjectAttributes, IoStatusBlock, ShareAccess, OpenOptions);
	}
